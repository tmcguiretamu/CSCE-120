...
// Function prototype
unsigned int moveDiscs(int, char, char, char);
...
int main()
{
   ...
   for (unsigned i = 0; i <= num_discs; i++)  {
      num_moves = 0;
      unsigned int moves = moveDiscs(i, FROM_PEG, TO_PEG, TEMP_PEG);
      cout << i << " discs:  total number of moves= " << moves << endl;
   }
   return 0;
}
...
unsigned int moveDiscs(int num, char fromPeg, char toPeg, char tempPeg)
{
